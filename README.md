...
播放长时间不推进时会尝试换歌，连续失败 8 次后停止自动重试；点击播放或按 `R` 可主动重试。切歌请求最多等待 20 秒，快速切歌或暂停会取消过期请求和恢复任务。浏览器检测到离线且播放失败时，会等待网络恢复后继续；手动暂停后不会自动恢复。

每 12 小时的曲库刷新会保留旧数据供播放使用。Jamendo 在完整抓取并保存新歌曲池后替换旧池；iTunes 按国家更新成功的榜单，失败的国家继续使用旧榜单。
...
Jamendo 模式从接口读取目录总量，在整个目录的随机位置在线抽取歌曲（包含单曲与专辑曲目），不限定为热门榜或预选的 360 首。最多保留 6 首待播候选，预加载其中 4 首；播放后移出候选并继续联网补充。当前会话最近 1000 首避免重复。地区资料是否完整不影响选歌。

新一批歌手的地区通过 Jamendo 艺人编号合并查询，与音频下载同时进行；缺失资料再由 MusicBrainz 补充。已核实地区仅是查询缓存，不用于决定播放哪些歌曲。随机范围是 Jamendo 当前可提供的歌曲，不代表全球所有平台的全部音乐，也不是各国家等概率。深分页受远端响应速度影响，首次启动或快速连续跳过可能需要等待。
